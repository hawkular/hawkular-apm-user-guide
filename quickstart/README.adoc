...
If using a Zipkin client library that requires Kafka, then prior to starting the Hawkular APM server ensure you have a Kafka server running and set the environment variable *KAFKA_ZOOKEEPER* to its IP address.
